...
export default function LoanNavbar() {
  ...
  useEffect(() => {
    ...
    // NOTE: Added 'gold' loan page check here
    else if (path.includes('gold-loan')) setCurrentPage('gold');
  ...
  const getGradient = () => {
    ...
    switch (currentPage) {
      ...
      case 'medical':
        ...
        // NOTE: Added 'gold' loan gradient here
      case 'gold':
        return 'bg-gradient-to-r from-yellow-50 via-amber-50 to-orange-50';
      default:
        return 'bg-gradient-to-r from-blue-50 via-cyan-50 to-indigo-50';
    }
  };
            ...
            <li className="relative group" ref={loansDropdownRef}>
              ...
              {dropdown === 'loans' && (
                  <div className="absolute left-0 mt-2 w-56 bg-white rounded-lg shadow-lg py-2 z-30">
                    ...
                    {/* NEW: Gold Loan added to Desktop Dropdown */}
                    <Link href="/loans/gold-loan" className="block px-5 py-2 hover:bg-blue-50 text-gray-700">Gold Loan</Link>
                  </div>
              )}
            </li>
            ...
            <div className="md:hidden bg-white px-4 pb-6 rounded-b-xl shadow animate-fadeIn">
              <ul className="space-y-4 text-gray-800 font-medium text-base">
                ...
                <ul className="ml-4 space-y-1">
                  ...
                  {/* NEW: Gold Loan added to Mobile Menu */}
                  <li><Link href="/loans/gold-loan" onClick={closeMobileMenu} className="block py-1">Gold Loan</Link></li>
                </ul>
