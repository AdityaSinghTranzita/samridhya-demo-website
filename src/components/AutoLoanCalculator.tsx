...
import { Calculator, Car, Calendar, Percent, Target, BarChart3, TrendingUp } from 'lucide-react';
...
export default function AutoLoanCalculator({ 
  ...
  secondaryColor = "from-gray-50 to-orange-50",
  accentColor = "orange",
  id
}: AutoLoanCalculatorProps) {
  ...
  const [carPriceInput, setCarPriceInput] = useState(formatNumber(carPrice));
  const [downPaymentInput, setDownPaymentInput] = useState(formatNumber(downPayment));
  const [tradeInValueInput, setTradeInValueInput] = useState(formatNumber(tradeInValue));
  ...
  const calculateAutoLoan = () => {
    ...
    const loanAmount = carPrice - totalDownPayment;
    ...
    const monthlyIncome = 100000; // Assuming ₹1L monthly income
    const maxEMI = monthlyIncome * 0.3; // 30% of monthly income
  ...
  const handleCalculate = () => {
    setShowResults(true);
    trackCalculatorUsage('auto_loan', {
      car_price: carPrice,
    ...
    trackButtonClick('calculate_auto_loan', 'auto_loan_calculator', {
      car_price: carPrice,
      down_payment: downPayment,
      trade_in_value: tradeInValue,
      interest_rate: interestRate,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Auto Loan Calculator</h2>
              <p className="text-gray-600">Calculate car loan payments and affordability</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  value={carPriceInput}
                  onChange={(e) => handleInputChange(e.target.value, setCarPrice, setCarPriceInput)}
                  className="w-full pl-12 pr-4 py-4 border border-gray-300 rounded-2xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(e.target.value, setDownPayment, setDownPaymentInput)}
                  className="w-full pl-12 pr-4 py-4 border border-gray-300 rounded-2xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(e.target.value, setTradeInValue, setTradeInValueInput)}
                  className="w-full pl-12 pr-4 py-4 border border-gray-300 rounded-2xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-300"
        ...
        >
          ...
          {showResults && (
            <motion.div
              className="bg-gradient-to-br from-orange-600 to-red-600 rounded-3xl p-8 text-white shadow-xl"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.4 }}
            >
              <h3 className="text-xl font-semibold mb-6">Auto Loan Summary</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-orange-100">Monthly EMI</span>
                  <span className="text-2xl font-bold">{formatCurrency(results.emi)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-orange-100">Loan Amount</span>
                  <span className="text-lg font-semibold">{formatCurrency(results.loanAmount)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-orange-100">Total Interest</span>
                ...
                <div className="pt-4 border-t border-orange-500">
                  <div className="flex justify-between items-center">
                    <span className="text-orange-100">Total Payment</span>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center py-3 border-b border-gray-100">
                  ...
                  <span className="font-semibold text-gray-900">{formatCurrency(30000)}</span>
                </div>
                <div className="flex justify-between items-center py-3 bg-orange-50 rounded-xl px-4">
                  <span className="text-orange-700 font-semibold">Affordability Score</span>
                  <span className={`font-bold text-lg ${results.affordability > 100 ? 'text-red-600' : 'text-orange-600'}`}>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-orange-50 rounded-2xl">
                  <div className="text-2xl font-bold text-orange-600">{formatCurrency(results.loanAmount)}</div>
                  <div className="text-sm text-gray-600">Loan Amount</div>
                </div>
                <div className="text-center p-4 bg-red-50 rounded-2xl">
                  <div className="text-2xl font-bold text-red-600">{interestRate}%</div>
              ...
              <tbody>
                {yearlyBreakdown.map((item, index) => (
                  <tr key={item.year} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-100`}>
                    ...
                    <td className="py-3 px-4 text-right font-medium text-orange-600">{formatCurrency(item.yearPrincipal)}</td>
